...
import { Activity, AlertTriangle, Archive, ClipboardCopy, FileSpreadsheet, FileText, Target } from 'lucide-react';
    ...
    <div className="space-y-5">
      ...
      >
        <div className="grid gap-3 md:grid-cols-4">
          <SelectField label="Vykazované období" value={dashboardFilters.period} onChange={(value) => setDashboardFilters((prev) => ({ ...prev, period: value }))} options={REPORTING_PERIODS.map((period) => ({ value: period.value, label: period.label }))} />
      ...
      <section>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="rounded-lg border border-slate-300 bg-slate-100 p-4">
            <div className="mb-3 flex items-center gap-2"><Target className="h-4 w-4 text-indigo-600" /><h3 className="text-sm font-bold text-slate-900">Dlouhodobá podpora – klienti 40+ hodin</h3></div>
      ...
      <section>
        <h2 className="mb-3 text-base font-bold text-slate-900">Kontrolní upozornění</h2>
