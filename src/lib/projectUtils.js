...
function mapSheetRowToClient(row, index) {
  ...
  if (Array.isArray(row)) {
    ...
    return enrichClient({
      ...
      datumNarozeni: formatDate(row.datum_narozeni),
      ...
      datumVstupu: formatDate(row.datum_vstupu_do_projektu),
      datumVystupu: formatDate(row.datum_vystupu_z_projektu),
      ...
      caseManagementOd: formatDate(row.case_management_od),
      poznamka: row.poznamka || '',
      situacePoUkonceni: row.situace_po_ukonceni || '',
      driveFolderUrl: row.drive_folder_url || '',
      monitoringListUrl: row.monitoring_list_url || '',
      projectStatus: status.startsWith('ukon') ? 'completed' : 'active'
    });
