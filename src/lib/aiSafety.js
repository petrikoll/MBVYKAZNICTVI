...
function validateRecordOutput(output, { consultationType, client } = {}) {
  ...
  if (String(output.consultationType || '') !== String(consultationType || '')) throw new Error('AI změnila uzamčený typ podpory.');
  if (containsClientIdentifier(recordText, client)) throw new Error('AI výstup obsahuje zakázaný osobní údaj klienta.');
  return { recordText, warnings: Array.isArray(output.warnings) ? output.warnings.map(String) : [] };
}
