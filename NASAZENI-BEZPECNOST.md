...
4. Přidejte vlastnost `SPREADSHEET_ID` s ID produkční Google tabulky. Tato hodnota má přednost před vývojovým ID uvedeným ve zdrojovém souboru a brání nasazení proti nesprávné databázi.
5. Nasaďte novou verzi Apps Scriptu.
6. Na Renderu nastavte neveřejné proměnné:
...
7. Nasaďte novou verzi aplikace.
8. Po ověření lze z Renderu odstranit staré proměnné `VITE_CLIENTS_API_URL` a `VITE_CLIENTS_API_TOKEN`.
